...
data Event (muxMode :: MuxMode) peerAddr m a b
    ...
    --
    | DemotedToColdRemote    !(DemotedToColdTransition peerAddr)
...
--
data DemotedToColdTransition peerAddr =
  ...
  | CommitRemote   !(ConnectionId peerAddr)
    ...
    -- | Transition from 'RemoteIdle' to 'RemoteEstablished'.
    --
...
firstPeerDemotedToCold
    :: MonadSTM m
    => InboundGovernorState muxMode peerAddr m a b
    -> STM m (DemotedToColdTransition peerAddr)
        ...
        case csRemoteState of
          ...
          RemoteEstablished ->
            ...
            in
                  outer
              <|> (Map.foldlWithKey'
                    (\inner (_, miniProtocolDir) miniProtocolStatusVar ->
                      case miniProtocolDir of
                        InitiatorDir -> inner
    ...
    inboundGovernorLoop !state = do
      event
        <- atomically $
            ...
            <|> (PromotedToWarmRemote   <$> firstPeerPromotedToWarm state)
            <|> (DemotedToColdRemote    <$> firstPeerDemotedToCold state)
      ...
      case event of
        ...
        DemotedToColdRemote (WaitIdleRemote connId) -> do
          -- @
        ...
        DemotedToColdRemote (AwakeRemote connId) -> do
        ...
        DemotedToColdRemote (CommitRemote connId) -> do
