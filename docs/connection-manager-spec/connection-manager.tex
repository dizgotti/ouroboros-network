...
\subsubsection{Closing a TCP connections}
...
specified liner timeout fires.  Unfortuantelly, our experiments showed that if
...
\subsection{\texttt{demotedToColdRemote}}
...
\subsection{Connection manager methods and transition graph}
\begin{figure}[p]
  {\hspace{-3cm}\begin{tikzpicture}[scale=0.66]
    \node                                     (init)                       at ( 2,   2)     {\small\InitialState};
    \node[outbound_state]                     (ReservedOutboundState)      at ( 0,-0.25)    {\small\ReservedOutboundState};
